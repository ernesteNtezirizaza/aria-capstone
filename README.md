...
- **Boundary/edge-case testing**: forced battery to its critical threshold to verify the emergency-response path. An earlier build branched this on weather (sunny cancelled an in-progress critical return, rainy forced a full return-and-land); a later full parity audit against `rwanda_env.py` found that split had no basis in the trained environment, which terminates on critical battery immediately and unconditionally regardless of weather, and removed it (see Discussion). The fix is not yet re-verified live, pending a new WebGL build.
...
**Obstacle Detection & Avoidance**: hazards (red markers) cluster along real steep-slope ridgelines, since the obstacle map is derived from actual DEM slope and elevation-turbulence data, not placed arbitrarily, and they are visible immediately with zero clicks, since obstacles are always the real hazard grid rather than a manually-toggled overlay:
...
**Unattended 14-Minute Run, Mission Complete Before Exhaustion**: same untouched session extended to 14 minutes; the mission finished (499/500 seeds) with battery to spare, then the drone landed and battery recovered under solar while grounded:
![Battery mission complete untouched](docs/testing/screenshots/11-battery-mission-complete-untouched.png)
